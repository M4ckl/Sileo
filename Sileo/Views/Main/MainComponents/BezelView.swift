        ...
        ZStack {
            ...
            ZStack {
                ...
                Circle()
                    ...
                    .blur(radius: 4)
                    .offset(x: 2, y: 2)
                    .mask(Circle().stroke(Color.black, lineWidth: 6))
            }
            .frame(width: size, height: size)
            .opacity(isSleepMode ? 0.2 : 1.0)

            ZStack {
                Circle().fill(theme.accentColor.opacity(isSleepMode ? 0.8 : 0.6))
                Circle().fill(Color.white).opacity(isSleepMode ? 0.4 : 1.0)
